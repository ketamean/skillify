...
function CourseVideoSection(props: CourseVideoSectionProps) {
  ...
  return (
    <div className="w-full flex flex-col gap-y-4">
      <h3
        className={`bg-[#ededed] py-2 px-4 flex gap-x-2 ${
          props.index === 0 ? "rounded-t-xl" : ""
        }`}
      >
        ...
        <tbody>
          {props.videos?.map((video, index) => {
            ...
            return (
              <tr
                key={index}
                className={`w-full min-w-full flex flex-row gap-x-4 border-b-1 border-b-zinc-300 py-2 ${
                  video.visibility ? "cursor-pointer" : ""
                }`}
                onClick={
                  video.visibility
                    ? () => {
              ...
              >
                <td className="w-6 h-full">
                  {video.visibility ? (
                    <FontAwesomeIcon
                      icon={faPlay}
                      className={`text-[${iconColor}] w-full aspect-square`}
                    />
                  ) : (
                    <FontAwesomeIcon
                      icon={faLock}
...
export default function CourseDetails(props: CourseDetailsProps): ReactElement {
  ...
  useEffect(() => {
    ...
    if (paymentStatus === "success") {
      ...
      setMessage(
        "Payment succeeded, but an error occurred during enrollment. Please contact support."
      );
  ...
  async function handleCheckout() {
    try {
      ...
      // For paid courses, continue with Stripe checkout
      if (priceToCheck < 13000 && priceToCheck > 0) {
      ...
      if (priceToCheck !== 0) window.location.href = data.url;
  ...
  return (
    <div className="w-full h-full flex flex-col">
      {/* header */}
      <div
        className="bg-deepteal w-full h-full flex flex-col gap-y-8 pt-2 pb-8 px-0
                md:py-12"
      >
        <div
          className="flex flex-col gap-y-8 text-white w-full h-full px-0
          ...
          <div className="w-full h-full flex flex-col gap-y-4 px-6 lg:px-0">
            ...
            <div className="w-full h-full px-0 flex flex-col gap-y-1">
              <div className="w-full flex flex-row gap-x-2">
                {typeof props.numberOfEnrollments === "number" &&
                !isNaN(props.numberOfEnrollments) ? (
                  <>
                    ...
                    <div>
                      <span>
                        {props.numberOfEnrollments} student
                        {props.numberOfEnrollments >= 2 ? "s" : ""}
                      </span>
                    </div>
                  </>
                ) : (
              ...
              <div>
                Created by{" "}
                <a href={props.linkToInstructorPage}>
                  <span className="text-white underline hover:text-zinc-400!">
      ...
      {/* body */}
      <div
        className="text-black py-8 px-4 flex flex-col gap-y-8 text-justify
                md:px-40"
      >
        {/* related topics */}
        {props.relatedTopics ? (
          <div className="flex flex-col gap-y-4">
            ...
            <div className="flex flex-row gap-x-6">
              {props.relatedTopics.map((topic) => (
                <a
                  className="py-2 px-4 text-lg text-black! font-medium cursor-pointer border-1 border-zinc-500 rounded-lg hover:border-zinc-500! hover:bg-zinc-200"
                  href={topic.link}
                  target="_blank"
                >
                  {topic.name}

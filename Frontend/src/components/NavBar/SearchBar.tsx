...
export default function SearchBar(props: SearchBarProps): ReactElement {
  const [searchIconState, setSearchIconState] = useState<boolean>(
  ...
  return (
    <form
      className={`w-full max-w-full h-full max-h-full relative focus:outline-none focus:border-none items-center ${
        props.textColor ? `text-[${props.textColor}]!` : "text-white!"
      }`}
      onSubmit={props.onSubmit ? props.onSubmit : handleSearch}
    >
      <input
        className="w-full h-3/4 min-h-12 text-nowrap overflow-hidden pl-4 pr-12 mt-2 rounded-full border-1
                focus:outline-none inline relative"
        placeholder="Search for anything..."
        onInput={(e: FormEvent<HTMLInputElement>) => {
          const input: HTMLInputElement | null = e.target as HTMLInputElement;
          if (!input) return;
          if ((input.value as string).trim()) setSearchIconState(false);
      ...
      <button
        ...
        className={`h-[calc(75%-0.5rem)] aspect-square absolute right-1 top-[calc(12.5%+0.25rem)] p-2 rounded-full ${
          props.borderColor ? `border-[${props.borderColor}]!` : "border-white"
        }
                    ${
                      searchIconState
                        ? `cursor-not-allowed ${
                            props.disabledButtonColor
                              ? `bg-[${props.disabledButtonColor}]`
                              : "bg-zinc-500"
                          }`
                        : `cursor-pointer ${
                            props.enabledButtonColor
                              ? `bg-[${props.enabledButtonColor}]`
                              : "bg-light-teal"
                          }`
                    }
                `}
      >
        <SearchIcon
          onClick={(e: Event): void => {
            ...
            // else
            const res: HTMLElement | null = getNearestParentByTagName(

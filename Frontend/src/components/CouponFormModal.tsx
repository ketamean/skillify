...
const CouponFormModal: React.FC<CouponFormModalProps> = ({
  ...
  const [discountType, setDiscountType] = useState<
    "PercentageBased" | "ValueBased"
  >("PercentageBased");
  const [maxUsage, setMaxUsage] = useState(1);
  const [expirationTime, setExpirationTime] = useState("");
  const [formErrors, setFormErrors] = useState<string[]>([]);
  const [couponType, setCouponType] = useState<
    "ParticularInstructor" | "ParticularCourse" | "Universal"
  >("ParticularInstructor");
  ...
  useEffect(() => {
    ...
    const applyInitialData = async () => {
      ...
      setDiscountType(
        initialData.discount_type as "PercentageBased" | "ValueBased"
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === "edit" && initialData) {
        ...
        //     instructor_id: user.id,
        // });
        const res = await axiosForm.post("/api/coupons", payload);

...
export default function CoursePage(props: CoursePageProps) {
    ...
    useEffect(() => {
                ...
                (res) => {
                    ...
                    setCourseData((prev) => {
                        ...
                        const r: CourseDetailsProps = {
                            ...
                            linkToInstructorPage: res.data[0].instructors.id ? `/user?id=${res.data[0].instructors.id}` : '#',
                            instructorName: `${res.data[0].instructors.first_name? res.data[0].instructors.first_name : ""} ${res.data[0].instructors.last_name? res.data[0].instructors.last_name : ""}`
                        }
                        return prev?
                            {
                                ...prev,
        ...
        supabase
            .rpc('get_sections_with_videos_by_course', { courseid: props.id })
            .then(
                (res) => {
                    if (res.error) {
                        console.log(res.error)
